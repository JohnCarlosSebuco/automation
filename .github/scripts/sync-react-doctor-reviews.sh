...
# Helper function: Calculate SHA-256 hash of react-doctor comment content
calculate_content_hash() {
  ...
  local combined="${sorted_inline}${summary_body}"
...
for PR_B64 in $PRS; do
  ...
  ALL_INLINE_COMMENTS_RAW=$(gh api "repos/${UPSTREAM_REPO}/pulls/${PR_NUMBER}/comments" --paginate \
    | jq -s "add // [] | [.[] | select(.user.login == \"${BOT_LOGIN}\") | select(.body | contains(\"React Doctor\"))]")
  ...
  # Find the most recent React Doctor summary comment
  SUMMARY_BODY=""
  while IFS= read -r ic; do
    IC_BODY=$(echo "$ic" | jq -r '.body')
    if echo "$IC_BODY" | grep -q 'react-doctor:summary'; then
      SUMMARY_BODY="$IC_BODY"
      # Don't break - keep updating to get the latest comment
    fi
  done < <(echo "$ALL_ISSUE_COMMENTS" | jq -c '.[]')

  if [ -z "$SUMMARY_BODY" ]; then
    echo "No React Doctor summary found for PR #${PR_NUMBER}. Skipping."
    continue
  fi

  # Extract error count from summary
  ERROR_COUNT=$(echo "$SUMMARY_BODY" | grep -o '[0-9]* errors' | sed 's/ errors//' | head -1 || echo "0")

  # Skip if no errors (only warnings)
  if [ "$ERROR_COUNT" -eq 0 ]; then
    echo "React Doctor found no errors (warnings only). Skipping."
    continue
  fi
  ...
  METADATA="<!-- REACT_DOCTOR_METADATA
PR_NUMBER: ${PR_NUMBER}
...
-->"
  ...
  for ATTEMPT in 1 2 3; do
    if gh issue comment "$ISSUE_NUMBER" --repo "$DEV_REPO" --body-file "$SUMMARY_FILE"; then
      break
    fi
    echo "Retry ${ATTEMPT}/3 for summary comment..."
    sleep $((ATTEMPT * 2))
  done
  rm -f "$SUMMARY_FILE"
  ...
  echo "Posted ${POSTED} comment(s) (summary + ${COMMENT_COUNT} inline) to issue #${ISSUE_NUMBER}."
